...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    try {
      ...
      let fraudDetectionResponse: Response;
      try {
        fraudDetectionResponse = await fetch(`${FASTAPI_URL}/api/detect`, {
          method: 'POST',
          headers: {
        ...
        if (msg.includes('fetch failed') || msg.includes('ECONNREFUSED') || msg.includes('timeout')) {
          throw new Error(
            ...
            `Make sure FASTAPI_URL env var is set to your deployed backend URL. ` +
            `Original error: ${msg}`
          );
        }
        throw fetchError;
      ...
      if (!fraudDetectionResponse.ok) {
        const errorBody = await fraudDetectionResponse.text().catch(() => '');
        throw new Error(`FastAPI error ${fraudDetectionResponse.status}: ${fraudDetectionResponse.statusText}. Body: ${errorBody.substring(0, 200)}`);
      }
